...
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///jaithari.db')
...
<body class="flex flex-col items-center justify-start pb-24 pt-3 px-3 sm:px-5">
    ...
    <script>
        ...
        for (let i = 0; i < 12; i++) { setTimeout(createParticle, i * 300); }
...
def home():
    ...
    theme = get_setting('theme', 'rose')
    theme_config = THEMES.get(theme, THEMES["rose"])
    ...
    confessions = Post.query.filter_by(post_type='confession').order_by(Post.created_at.desc()).all()
    study_posts = Post.query.filter_by(post_type='study').order_by(Post.created_at.desc()).all()
    ...
    return render_template_string(HTML_TEMPLATE,
        ...
        theme_text=theme_config["text"], theme_glass=theme_config["glass"], theme_border="rgba(244, 63, 94, 0.25)",
        theme_border_bright="rgba(244, 63, 94, 0.9)", theme_glow="rgba(244, 63, 94, 0.4)", theme_gradient=theme_config["gradient"],
        themes=THEMES
    )
...
def get_messages():
    msgs = Message.query.filter_by(is_deleted=False).order_by(Message.created_at.desc()).limit(50).all()
    return jsonify({"messages": [m.to_dict() for m in reversed(msgs)]})

@socketio.on('user_joined')
def user_joined(data):
    user = User.query.filter_by(ip_address=request.remote_addr).first()
    if user and request.remote_addr not in online_users: online_users[request.remote_addr] = user.name
...
def handle_message(data):
    user = User.query.filter_by(ip_address=request.remote_addr).first()
    if not user: return
    msg = Message(user_id=user.id, content=data.get('msg', ''))
...
def delete_message(data):
    ...
    if msg: msg.is_deleted = True; db.session.commit(); emit('message_deleted', {'msg_id': msg.id}, broadcast=True)

@socketio.on('clear_chat')
def clear_chat():
    user = User.query.filter_by(ip_address=request.remote_addr).first()
    if not user or not user.is_admin: return
    Message.query.update({Message.is_deleted: True}); db.session.commit()
    emit('chat_cleared', broadcast=True)

@socketio.on('disconnect')
def user_disconnected():
    if request.remote_addr in online_users: del online_users[request.remote_addr]
    emit('update_online_users', {'count': len(online_users), 'users': list(online_users.values())}, broadcast=True)
